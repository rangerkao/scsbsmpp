...
<%
	...
	ArrayList<PointRecord> list = pointDAO.getPointRecords();
	...
	if( action != null && action.length() > 0 )
	{
		if( action.equals("search") )
		{
			...
			if((request.getParameter("stype") != null ))
			{
				...
				list = pointDAO.getPointRecords(request.getParameter("stype"),str);
			}
		}
	}
	else
	{
		list = pointDAO.getPointRecords();
	}
    %>
...
<div id="wrap">
	...
	<div id="content">
      ...
      <input type="radio" onclick="searchshow(this.value);" name="stype" value="3" <% if ( request.getParameter("stype") != null && request.getParameter("stype").equals("2") ) out.println("checked"); %>>依公司名稱<br/>
	...
	<table border="5" cellpadding="8" frame="void" rules="none" width="100%" >
		<tr>
			<th>編號</th><th>儲值編號</th><th>加值點數</th><th>加值費用</th><th>加值時間</th><th>交易說明</th><th>公司名稱</th><th>會員編號</th>
		</tr>
		<%
		int point_num=1;
		...
		%>
			<tr>
				...
				<td><%=p.getAddPrice() %></td>
				<td><%=p.getAddTime2() %></td>
				<td><%=p.getAdd_remark() %></td>
				<td><%=p.getSmppuser_com() %></td>
